<#
.SYNOPSIS
    Step 2: Install SCCM prereqs and WSUS on A-MPDP. Run on Host A.
    Note: MP/DP/SUP roles are added later from A-SCCM console (cross-VM operation).
    Prereqs: A-MPDP domain-joined.
#>
[CmdletBinding()]
param(
...
Write-LabLog 'Installing SCCM prerequisite features...' -Step 'A-MPDP'
...
# ── Install WSUS ─────────────────────────────────────────────────────────────
Write-LabLog 'Installing WSUS...' -Step 'A-MPDP'
Invoke-LabRemote -IPAddress $IP -Credential $creds.Domain -TimeoutSec 600 -ScriptBlock {
...
Write-LabLog 'A-MPDP Step 2 complete.' -Level SUCCESS -Step 'A-MPDP'
