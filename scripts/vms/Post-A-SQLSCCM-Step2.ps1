<#
.SYNOPSIS
    Step 2: Install SQL 2019 on A-SQLSCCM. Run on Host A.
    Prereqs: A-SQLSCCM domain-joined, media share accessible (cmdkey for host creds).
#>
[CmdletBinding()]
param(
...
# ── Format data disk ─────────────────────────────────────────────────────────
Write-LabLog 'Initializing data disk as D:...' -Step 'A-SQLSCCM'
...
# ── Install SQL 2019 ─────────────────────────────────────────────────────────
Write-LabLog 'Installing SQL Server 2019 on A-SQLSCCM...' -Step 'A-SQLSCCM'
...
Invoke-LabRemote -IPAddress $IP -Credential $creds.Domain -TimeoutSec 1800 -ScriptBlock {
    ...
    # Auto-start services, restart to apply
...
Write-LabLog 'A-SQLSCCM Step 2 complete.' -Level SUCCESS -Step 'A-SQLSCCM'
